...
## 48. Migration 046 — Mercredi : SLDL, amplitude plafonnée par butée au genou

> Le user refuse d'« entraîner ses lombaires » (peur de se les niquer). Le SLDL n'entraîne pas le rachis en mouvement — c'est une **charnière de hanche**, lombaires **isométriques** (même catégorie que le plank, en anti-flexion au lieu d'anti-extension). Mais son instinct pointe un vrai risque : à 1m97 avec stockage abdominal, le **bas de l'amplitude** est exactement là où il s'arrondit (ischios courts + bras de levier énorme). Le risque n'est pas l'exercice, ce sont les 20 derniers centimètres.
>
> Le rack réglable (cf. 045) permet de supprimer cette portion : **barres de sécurité calées à hauteur de genou = butée physique**, impossible d'entrer dans la zone à risque. Bénéfice conservé (ischios, fessiers, gainage isométrique), portion dangereuse supprimée. Amplitude réduite **volontaire**, pas une dégradation — c'est ce qu'on ferait pour n'importe quel gabarit similaire.
>
